...
class MobileNetV3(nn.Module):
    def __init__(self, n_class=1000, input_size=224,
        ...
        initial_module = nn.Sequential(nn.Conv2d(1, input_channel, 3, 2, 1, bias=False),
        ...
        self.conv_stem = nn.Conv2d(1, 16, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
...
class IDAONet(nn.Module):
    def __init__(self, device):
        super().__init__()
        self.model = MobileNetV3(n_class=0).to(device)
        self.classifier = nn.Sequential(
            nn.Linear(1280, 256),
            nn.ReLU(),
            nn.Linear(256, 1),
    ...
    def forward(self, x):
        x = self.model.forward(x).squeeze()
        # x = self.head(x)
